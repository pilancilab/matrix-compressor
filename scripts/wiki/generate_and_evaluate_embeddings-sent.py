...
def embed(model, dataset, split="train", batch_size=1024, device=torch.device("cuda"), pool=None):
    data = dataset[split]
...
def parse():
    ...
    parser.add_argument("--dataset", default="wikitext-2-v1", choices=["wikitext-2-v1", "wikitext-103-v1"], help="Dataset to use for classifications")
...
train_data = embed(model, ds, "train", batch_size=batch_size, device=model_device, pool=pool)
logger.info(f"Train Data Text: {train_data.shape}")
logger.trace(f"Finished train embeddings")

logger.trace(f"Embedding test dataset")
test_data = embed(model, ds, "test", batch_size=batch_size, device=model_device, pool=pool)
logger.info(f"Test Data Text: {test_data.shape}")
...
dummy_labels = torch.empty(X.shape)
...
knn_lplr.fit(X_lplr, dummy_labels)
