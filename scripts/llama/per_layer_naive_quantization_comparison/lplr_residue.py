    ...
    for f in model_files:
        ...
        for name, weights in layer_set.items():
            ...
            if weights.ndim == 2:
                ...
                # row_sketch = lplr(weights.T - naive_quant.T, output_rank, b1, b2)
                # col_sketch = lplr(weights - naive_quant, output_rank, b1, b2)

                # row_sketch_err = relative_tensor_error(weights.T, row_sketch + naive_quant.T)
                # col_sketch_err = relative_tensor_error(weights, col_sketch + naive_quant)
