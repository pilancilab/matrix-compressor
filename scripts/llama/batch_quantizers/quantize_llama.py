...
def main(
    ...
    with torch.no_grad():
        ...
        for name, layer in tqdm(model.named_parameters()):
            ...
            assumed_rank = ceil(f * min(param.shape))
            if param.ndim == 2:
                layer.data = lplr(param, r=assumed_rank, B1=b1, B2=b2)
            logger.debug(
                f"Quantizing layer: {name} with shape {layer.shape} on device {layer.device}"
            )
